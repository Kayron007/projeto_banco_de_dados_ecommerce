...
-- NOTA: SUBSTITUIR OS 'ex' PELOS VALORES REAIS;
...
UPDATE pagamento
SET Status = 'pago'
WHERE ID_pagamento = 2 -- ID_ex
	AND (Status IS NULL OR Status = 'pendente');

-- atualizar pedido ligado ao pagamento
UPDATE pedido
SET Status = 'pagamento efetuado', fk_Pagamento_ID_pagamento = 2 -- ID_ex
WHERE fk_Pagamento_ID_pagamento = 2; -- ID_ex
...
WHERE fk_Fornecedor_ID_fornecedor = 2 -- ID_ex 
	AND fk_Produto_ID_produto = 22; -- ID_ex
...
SET Quantidade = 3, preco_unitario = 49.90 -- Exs
WHERE fk_ID_pedido = 41 -- ID_ex
	AND fk_ID_produto = 1; -- ID_ex
...
-- IREI COMEÇAR COM  ALGUNS COMANDOS, EXPLICANDO O QUE FAZEM E COMO USAR. FAREI 3 EXEMPLOS DE CADA. --

-- DESCRIBE --

DESCRIBE CLIENTE; -- COM O DESCRIBE PODEMOS VER A ESTRUTURA E SEUS VALORES DENTRO DA TABELA 'CLIENTE'.

DESCRIBE AVALIACAO; -- COM O DESCRIBE PODEMOS VER A ESTRUTURA E SEUS VALORES DENTRO DA TABELA 'AVALIACAO'.

DESCRIBE TELEFONE; -- COM O DESCRIBE PODEMOS VER A ESTRUTURA E SEUS VALORES DENTRO DA TABELA 'TELEFONE,'

-- OU SEJA, O DESCRIBE NOS MOSTRA A ESTRUTURA E SEUS VALORES INTENROS DENTRO DE CADA TABELA.

-- SELECT * FROM --

SELECT * FROM CLIENTE; -- AQUI MOSTRAMOS TODOS OS DADOS DE CADA UM DOS CLIENTES DA TABELA 'CLIENTE'

SELECT * FROM FORNECEDOR; -- AQUI MOSTRAMOS TODOS OS DADOS DE CADA UM DOS FORCEDORES DA TABELA 'FORNECEDOR'

SELECT * FROM PEDIDO; -- AQUI MOSTRAMOS TODOS OS DADOS DE CADA UM DOS PEDIDOS DA TABELA 'PEDIDO'

-- SEGUINDO A MESMA LOGICA PODE SE FAZER CONSULTAS COMPLETAS DE CADA TABELA JÁ CRIADA USANDO O COMANDO FROM * SELECT.

-- SELECT * FROM ORDER BY --
-- AGORA VAMOS FILTRAR O SELECT USANDO 'ORDER BY'

SELECT * FROM CLIENTE -- SELECIONAMOS TODOS DA TABELA CLIENTE 
ORDER BY NOME; -- E ORDENAMOS POR NOME (AO DEIXAR EM BRANCO OU COM ASK APÓS 'NOME' APARECERÁ TODOS OS CLIENTES DE FORMA CRESCENTE 'A' ATÉ 'Z').

SELECT * FROM CLIENTE -- SELECIONAMOS TODOS DA TABELA CLIENTE
ORDER BY ID_cliente; -- E ORDENAMOS POR ID_cliente ASSIM APARECERA TODOS OS CLIENTES E SEUS RESPECTIVOS IDs.

SELECT * FROM CLIENTE -- SELECIONAMOS TODOS DA TABELA CLIENTE
ORDER BY id_pessoa; -- E ORDENAMOS POR ID_cliente ASSIM APARECERA TODOS OS CLIENTES E SEUS RESPECTIVOS IDs.

SELECT * FROM CLIENTE -- SELECIONAMOS TODOS DA TABELA CLIENTE 
ORDER BY NOME DESC; -- E ORDENAMOS POR NOME MAS EM ORDEM DECRESENTE, ASSIM APARECERÁ TODOS OS CLIENTES DA LETRA 'Z' ATÉ 'A'

SELECT * FROM CLIENTE -- SELECIONAMOS TODOS DA TABELA CLIENTE
ORDER BY ID_cliente DESC; -- E ORDENAMOS POR ID_cliente ASSIM APARECERA TODOS OS CLIENTES E SEUS RESPECTIVOS IDs DE FORMA DECRESCENTE.

SELECT * FROM CLIENTE -- SELECIONAMOS TODOS DA TABELA CLIENTE
ORDER BY id_pessoa DESC; -- E ORDENAMOS POR ID_cliente ASSIM APARECERA TODOS OS CLIENTES E SEUS RESPECTIVOS IDs DE FORMA DECRESENTE.

-- SELECT SEM ASTERISCO * COM ORDER BY --

SELECT NOME, CEP FROM CLIENTE
ORDER BY NOME; -- SELECIONAMOS ASSIM A COLUNA NOME E CEP E ORDENAOS QUE ELA SIGA NOME.

SELECT DESCRICAO, PRECO FROM PRODUTO
ORDER BY DESCRICAO; -- SELECIONAMOS A COLUNA DE DESCRICAO E PRECO ORDENAMOS ELA PELA DESCRICAO DO PRODUTO.

SELECT PRECO, DESCRICAO, TAMANHO FROM PRODUTO
ORDER BY PRECO; -- SELECIONAMOS NOVAMENTE PRECO, DESCRICAO E AGORA SELECIONAMOS O TAMANHO. VEJA QUE DEFINI A ORDEM COMO O PRECO.

SELECT PRECO, DESCRICAO, TAMANHO FROM PRODUTO
ORDER BY PRECO, DESCRICAO; -- SELECIONAMOS NOVAMENTE PRECO, DESCRICAO E TAMANHO. MAS ORDENEI POR NOME E LOGO APOS POR DESCRICAO.

-- AGORA VAMOS ADICIONAR WHERE -

-- SELECT * FROM WHERE ORDER BY --

SELECT * FROM PEDIDO
WHERE STATUS = 'pagamento pendente' -- WHERE NA LEITURA SOA COMO 'ONDE' OU SEJA FILTRAMOS POR STATUS ENTÃO SÓ APARECEM PEDIDOS PENDENTES.
ORDER BY ID_pedido;

SELECT * FROM PAGAMENTO
WHERE STATUS = 'ESTORNADO' -- FILTREI POR STATUS NOVAMENTE SÓ QUE EM PAGAMENTOS ENTÃO SÓ APARECEM PAGAEMNTOS ESTORNADOS.
ORDER BY Forma_de_pagamento;

SELECT * FROM PESSOA_JURIDICA
WHERE CNPJ = '11.111.111/0001-11' -- FILTREI DE FORMA MAIS RESTRITIVA VISANDO ENCONTRAR UMA PESSOA JURIDICA COM UM CNPJ ESPECIFICO.
ORDER BY CNPJ;

-- PODEMOS USAR O FILTRO NO FROM TAMBÉM -- 

SELECT CNPJ FROM PESSOA_JURIDICA
WHERE CNPJ = '11.111.111/0001-11' -- FILTREI DE FORMA MAIS RESTRITIVA VISANDO ENCONTRAR UMA PESSOA JURIDICA COM UM CNPJ ESPECIFICO.
ORDER BY CNPJ;

-- WHERE COM ALGORITIMO--

SELECT CATEGORIA, QUANTIDADE, PRECO FROM PRODUTO
WHERE PRECO < 3000 -- EM WHERE (ONDE) PODEMOS COLOCAR ALGUNS ALGORITIMOS COMO <,>-, =, =!, <=, =>. SERVE PRA FILTRAR O QUE QUEREMOS VER DA TABELA ESCOLHIDA.
ORDER BY CATEGORIA;

SELECT DESCRICAO, TAMANHO , PRECO FROM PRODUTO
WHERE TAMANHO = 'M'
ORDER BY DESCRICAO DESC, PRECO ASC;

-- WHERE COM BETWEEN --

SELECT ID_PEDIDO, STATUS, DATA_DO_PEDIDO, VALOR_TOTAL FROM PEDIDO
WHERE VALOR_TOTAL BETWEEN 200 AND 5000 -- AQUI CONSIGO DEFINIR NO VALOR_TOTAL QUE QUERO APENAS VALORES BETWEEN (ENTRE) 200 AND (E) 5000.
ORDER BY ID_PEDIDO, STATUS, VALOR_TOTAL;

SELECT ID_PEDIDO, STATUS, DATA_DO_PEDIDO, VALOR_TOTAL FROM PEDIDO
WHERE VALOR_TOTAL BETWEEN 10 AND 500 -- AQUI CONSIGO DEFINIR NO VALOR_TOTAL QUE QUERO APENAS VALORES BETWEEN (ENTRE) 10 AND (E) 500.
ORDER BY ID_PEDIDO, STATUS, VALOR_TOTAL;

SELECT ID_PEDIDO, STATUS, DATA_DO_PEDIDO, VALOR_TOTAL FROM PEDIDO
WHERE VALOR_TOTAL BETWEEN 200 AND 5000 -- AQUI EU DEFINE O VALOR_TOTAL QUE QUERO APENAS OS VALORES BETWEEN (ENTRE) 200 AND (E) 5000.
ORDER BY ID_PEDIDO, STATUS, VALOR_TOTAL;

SELECT ID_NOTA_FISCAL, DATA_DE_EMISSAO, VALOR_TOTAL, IMPOSTO FROM NOTAFISCAL
WHERE IMPOSTO BETWEEN 20 AND 40 -- DEFINE O IMPOSTO ENTRE 20 E 40 REAIS O SEU VALOR 
ORDER BY ID_NOTA_FISCAL DESC, CHAVE_DE_ACESSO; -- COMO EXPLIQUEI ORDENEI PELO ID EM FORMA DECRESCENTE (MAIOR P/ MENOR ) E DPOIS PELA CHAVE_DE_ACESSO EM FORMA CRESCENTE.

-- WHERE COM IN AO INVÉS DE BETWEEN --

SELECT ID_AVALIACAO, NOTA, COMENTARIO FROM AVALIACAO
WHERE NOTA IN (1, 2, 5) -- O IN (EM) SERVE PRA FILTRAR O QUE EU QUERO DENTRO DA COLUNA NOTA.
ORDER BY NOTA, ID_AVALIACAO;

SELECT ID_AVALIACAO, NOTA, COMENTARIO FROM AVALIACAO
WHERE NOTA IN (1, 4) -- O IN (EM) SERVE PRA FILTRAR O QUE EU QUERO DENTRO DA COLUNA NOTA.
ORDER BY NOTA, ID_AVALIACAO;
...
SELECT ID_PAGAMENTO, STATUS, FORMA_DE_PAGAMENTO FROM PAGAMENTO
WHERE ID_PAGAMENTO > 9 AND STATUS = 'PAGO' -- AND NADA MAIS É DO QUE "E". ONDE ID_PAGAMENTO SEJA MAIOR DO QUE 9 E STATUS IGUAL A PAGO.
ORDER BY ID_PAGAMENTO;

SELECT ID_PAGAMENTO, STATUS, FORMA_DE_PAGAMENTO FROM PAGAMENTO
WHERE ID_PAGAMENTO < 20 AND STATUS = 'PENDENTE' -- AND NADA MAIS É DO QUE "E". ONDE ID_PAGAMENTO SEJA MAIOR DO QUE 20 E STATUS IGUAL A PENDENTE.
ORDER BY ID_PAGAMENTO, FORMA_DE_PAGAMENTO;

SELECT ID_PEDIDO, STATUS, VALOR_TOTAL FROM PEDIDO
WHERE ID_PEDIDO > 45 AND VALOR_TOTAL < 300 -- AND NADA MAIS É DO QUE "E". ONDE ID_PEDIDO SEJA MAIOR DO QUE 45 E VALOR_TOTAL MENOR QUE 300.
ORDER BY STATUS DESC, ID_PEDIDO, VALOR_TOTAL;

-- SELECT FROM LIKE --

SELECT * FROM PEDIDO
WHERE STATUS LIKE 'P%'; -- LIKE É COMO UM FILTRO, NESSE CASO ELE SERVE PRA DIZER QUE QUER TODOS OS RESULTADOS DO STATUS QUE COMECEM COM A LETRA "P".

SELECT ID_PEDIDO, STATUS FROM PEDIDO
WHERE STATUS LIKE 'PA%E'; -- LIKE É COMO UM FILTRO, NESSE CASO ELE SERVE PRA DIZER QUE QUER TODOS OS RESULTADOS DO STATUS QUE COMECEM COM A LETRA "PA" E TERMINEM COM A LETRA "E".

SELECT ID_PEDIDO, STATUS FROM PEDIDO 
WHERE STATUS LIKE '%A%'; -- LIKE É COMO UM FILTRO, NESSE CASO ELE SERVE PRA DIZER QUE QUER TODOS OS RESULTADOS DO STATUS QUE CONTENHAM LETRA "A" DEPOIS DO INCIO E ANTES DO FINAL.

SELECT DATA_DO_PEDIDO, STATUS, VALOR_TOTAL FROM PEDIDO
WHERE STATUS LIKE 'E%AD_'; -- LIKE É COMO UM FILTRO, NESSE CASO ELE SERVE PRA DIZER QUE QUER TODOS OS RESULTADOS DO STATUS QUE COMECEM COM A LETRA "E" CONTENHA "AD" E TENHA ALGUM CARACTERE NO FINAL NUMERO OU LETRA.

-- SELECT FROM NOT LIKE --

SELECT DESCRICAO, QUANTIDADE, PRECO, CATEGORIA FROM PRODUTO
WHERE CATEGORIA NOT LIKE 'C%'; -- NOT LIKE É TAMBÉM UM FILTRO, NESSE CASO ELE ESTÁ DIZENDO QUE QUER OS RESULTADOS DA CATEGORIA QUE NÃO COMECEM COM "C".

SELECT DESCRICAO, QUANTIDADE, PRECO, CATEGORIA FROM PRODUTO
WHERE CATEGORIA NOT LIKE '%A%'; -- NOT LIKE É TAMBÉM UM FILTRO, NESSE CASO ELE ESTÁ DIZENDO QUE QUER OS RESULTADOS DA CATEGORIA QUE NÃO CONTENHA A LETRA "A" NO SEU MEIO.

SELECT DESCRICAO, QUANTIDADE, PRECO, CATEGORIA FROM PRODUTO
WHERE CATEGORIA NOT LIKE '%A'; -- NOT LIKE É TAMBÉM UM FILTRO, NESSE CASO ELE ESTÁ DIZENDO QUE QUER OS RESULTADOS DA CATEGORIA QUE NÃO TERMINEM COM "A".

-- SELECT COM DISTINCT --

SELECT DISTINCT STATUS FROM PAGAMENTO; -- DISTINCT COMO O PRÓPIO NOME SUGERE SERVE PRA DESTINGUIR PALAVRAS IGUAIS DE UMA COLUNA DENTRO DE UMA TABELA.


SELECT DISTINCT FORMA_DE_PAGAMENTO FROM PAGAMENTO; -- DISTINCT COMO O PRÓPIO NOME SUGERE SERVE PRA DESTINGUIR PALAVRAS IGUAIS DE UMA COLUNA DENTRO DE UMA TABELA.


SELECT DISTINCT FK_FORNECEDOR_ID_FORNECEDOR FROM FORNECEDOR_PRODUTO; -- DISTINCT COMO O PRÓPIO NOME SUGERE SERVE PRA DESTINGUIR PALAVRAS IGUAIS DE UMA COLUNA DENTRO DE UMA TABELA.

-- SELECT COM COUNT --

SELECT COUNT(*) FROM PEDIDO; -- COUNT SERVE PARA CONTAR TODAS AS LINHAS E MOSTRAR O VALOR TOTAL DAS LINHAS

SELECT COUNT(*) FROM PAGAMENTO; -- COUNT SERVE PARA CONTAR TODAS AS LINHAS E MOSTRAR O VALOR TOTAL DAS LINHAS

SELECT COUNT(*) FROM PESSOA_JURIDICA; -- COUNT SERVE PARA CONTAR TODAS AS LINHAS E MOSTRAR O VALOR TOTAL DAS LINHAS

SELECT COUNT(*) FROM PESSOA_JURIDICA -- COUNT SERVE PARA CONTAR TODAS AS LINHAS E MOSTRAR O VALOR TOTAL DAS LINHAS, AQUI USEI O WHERE PRA FILTRAR A PARTIR DO CNPJ MAIOR QUE O INFORMADO.
WHERE CNPJ > '77.777.777/0001-77';

SELECT COUNT(CNPJ) FROM PESSOA_JURIDICA; -- COUNT SERVE PARA CONTAR TODAS AS LINHAS E MOSTRAR O VALOR TOTAL DAS LINHAS

-- SELECT COM MIN, SUM, AVG E MAX --

SELECT * FROM NOTAFISCAL;

SELECT MAX(IMPOSTO) FROM NOTAFISCAL; -- MAX SERVE PRA MOSTRAR O MAIOR VALOR DE UMA DETERMINADA COLUNA DENTRO DE UMA TABELA

SELECT MAX(VALOR_TOTAL) FROM NOTAFISCAL; -- MAX SERVE PRA MOSTRAR O MAIOR VALOR DE UMA DETERMINADA COLUNA DENTRO DE UMA TABELA

SELECT MAX(VALOR_TOTAL) FROM NOTAFISCAL 
WHERE DATA_DE_EMISSAO LIKE '2025-09%'; -- MAX SERVE PRA MOSTRAR O MAIOR VALOR DE UMA DETERMINADA COLUNA DENTRO DE UMA TABELA, COM WHERE LIKE DETERMINAMOS ONDE E O QUE CONTÉM.

SELECT MIN(VALOR_TOTAL) FROM NOTAFISCAL; -- MIN SERVE PRA MOSTRAR O MENOR VALOR DE UMA DETERMINADA COLUNA DENTRO DE UMA TABELA.

SELECT MIN(VALOR_TOTAL) FROM NOTAFISCAL 
WHERE DATA_DE_EMISSAO LIKE '2025-09%'; -- MIN SERVE PRA MOSTRAR O MENOR VALOR DE UMA DETERMINADA COLUNA DENTRO DE UMA TABELA, COM WHERE LIKE DETERMINAMOS ONDE O QUE CONTÉM.

SELECT SUM(VALOR_TOTAL) FROM NOTAFISCAL; -- SUM É PRA SOMAR O VALOR TOTAL DE UMA DETERMINADA COLUNA EM UMA TABELA.

SELECT SUM(VALOR_TOTAL) FROM NOTAFISCAL 
WHERE DATA_DE_EMISSAO LIKE '2025-09%'; -- SUM É PRA SOMAR O VALOR TOTAL DE UMA DETERMINADA COLUNA EM UMA TABELA, COM WHERE LIKE FILTRAMOS ONDE E O QUE CONTÉM.

SELECT AVG(VALOR_TOTAL) FROM NOTAFISCAL; -- AVG É PRA SOMAR O MOSTRAR O VALOR DA MEDIA ARITMETICA DE UMA DETERMINADA COLUNA EM UMA TABELA.

SELECT AVG(VALOR_TOTAL) FROM NOTAFISCAL 
WHERE DATA_DE_EMISSAO LIKE '2025-10%'; -- AVG É PRA SOMAR O MOSTRAR O VALOR DA MEDIA ARITMETICA DE UMA DETERMINADA COLUNA EM UMA TABELA, COM WHERE LIKE FILTRAMOS ONDE E O QUE CONTÉM.

-- SELECT COM DISTINCT E ORDER BY--

SELECT DISTINCT FK_FORNECEDOR_ID_FORNECEDOR FROM FORNECEDOR_PRODUTO
ORDER BY FK_FORNECEDOR_ID_FORNECEDOR; -- O DISTINCT COM O ORDER BY ALÉM DE DESTINGUIR ELE TAMBÉM ORDENA DE FORMA CRESCENTE.

SELECT DISTINCT ID_NOTA_FISCAL FROM NOTAFISCAL
ORDER BY ID_NOTA_FISCAL; -- O DISTINCT COM O ORDER BY ALÉM DE DESTINGUIR ELE TAMBÉM ORDENA DE FORMA CRESCENTE.

SELECT DISTINCT IMPOSTO FROM NOTAFISCAL
ORDER BY IMPOSTO; -- O DISTINCT COM O ORDER BY ALÉM DE DESTINGUIR ELE TAMBÉM ORDENA DE FORMA CRESCENTE.

-- SELECT COM GROUP BY --

SELECT VALOR_TOTAL FROM NOTAFISCAL
GROUP BY VALOR_TOTAL; -- GROUP BY UNE VALORES IGUAIS EM UMA UNICA LINHA

SELECT VALOR_TOTAL, COUNT(*) FROM NOTAFISCAL
GROUP BY VALOR_TOTAL; -- GROUP BY UNE VALORES IGUAIS EM UMA UNICA LINHA, NESSE CASO COM O COUNT(*) ELE MOSTRA A QUANTIDADE DE LINHAS AGRUPADAS.

SELECT VALOR_TOTAL, COUNT(*), SUM(VALOR_TOTAL) FROM NOTAFISCAL
GROUP BY VALOR_TOTAL; -- NESSE CASO, ELE ALÉM DE MOSTRAR A QUANTIDADE DE LINHAS AGRUPADAS ELE TAMBÉM SOMA AS LINHAS QUE TEM AGRUPAMENTO E MOSTRA O VALOR SOMADO.

-- SELECT COM GROUP BY E HAVING --

SELECT COMENTARIO, COUNT(*) AS QTD_COMENTARIOS_IGUAIS FROM AVALIACAO -- "AS" SERVE PRA DAR UM NOME FICTICIO E COUNT SERVE PRA CONTAR A QUANTIDADE DE LINHAS IGUAIS NO GROUP BY
GROUP BY COMENTARIO -- GROUP BY SERVE PRA JUNTAR EM UMA UNICA LINHA RESULTADOS IGUAIS 
HAVING COUNT(COMENTARIO) >= 3 -- HAVING É TIPO UM WHERE PORÉM PRA GROUP BY
ORDER BY COUNT(*); -- ORDER BY É ORDENAÇÃO DE FORMA CRESCENTE OU DECRESCENTE

SELECT IMPOSTO, COUNT(*)  AS QTD_DE_IMPOSTOS_IGUAIS FROM NOTAFISCAL -- "AS" SERVE PRA DAR UM NOME FICTICIO E COUNT SERVE PRA CONTAR A QUANTIDADE DE LINHAS IGUAIS NO GROUP BY
GROUP BY IMPOSTO -- GROUP BY SERVE PRA JUNTAR EM UMA UNICA LINHA RESULTADOS IGUAIS 
HAVING COUNT(IMPOSTO) -- HAVING É TIPO UM WHERE PORÉM PRA GROUP BY
ORDER BY COUNT(*); -- ORDER BY É ORDENAÇÃO DE FORMA CRESCENTE OU DECRESCENTE

SELECT IMPOSTO, COUNT(*) AS QTD_DE_IMPOSTOS_IGUAIS FROM NOTAFISCAL -- "AS" SERVE PRA DAR UM NOME FICTICIO E COUNT SERVE PRA CONTAR A QUANTIDADE DE LINHAS IGUAIS NO GROUP BY
WHERE DATA_DE_EMISSAO >= '2025-09-20 00:00:00' -- WHERE É UM FILTRO QUE COMO EU JÁ DISSE SIGNIFICA "ONDE" 
GROUP BY IMPOSTO -- GROUP BY SERVE PRA JUNTAR EM UMA UNICA LINHA RESULTADOS IGUAIS 
HAVING COUNT(IMPOSTO) -- HAVING É TIPO UM WHERE PORÉM PRA GROUP BY
ORDER BY COUNT(*); -- ORDER BY É ORDENAÇÃO DE FORMA CRESCENTE OU DECRESCENTE

SELECT AVG(NOTA) FROM AVALIACAO; -- PARA EXEMPLO DE USO NO CÓDIGO ABAIXO, SELECT DENTRO DE OUTRO SELECT

SELECT NOTA, COUNT(*) AS QTD_NOTAS_IGUAIS FROM AVALIACAO -- "AS" SERVE PRA DAR UM NOME FICTICIO E COUNT SERVE PRA CONTAR A QUANTIDADE DE LINHAS IGUAIS NO GROUP BY
WHERE ID_AVALIACAO > 16  -- WHERE É UM FILTRO QUE COMO EU JÁ DISSE SIGNIFICA "ONDE" 
GROUP BY NOTA -- AGRUPAMENTO DA NOTA
HAVING NOTA > (SELECT AVG(NOTA) FROM AVALIACAO); -- FILTRO IGUAL O HERE É NO SELECT, ESSE SELECT DENTRO DO OUTRO SERVE PARA ATUALIZAR A NOTA AUTOMATICAMENTE.
